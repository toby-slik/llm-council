...
export default function WizardChat({ onEvaluationsComplete }) {
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: "bot",
      content: "Hi! I'm your Creative Effectiveness AI. To get started, please upload the creative asset(s) or brief you'd like to analyze.",
      type: "upload_initial"
    }
  ]);
  ...
  const handleInitialUpload = async (e) => {
    ...
    try {
      addBotMessage("Analyzing your asset(s) to establish the Contextual Baseline...", "loading");
  ...
  const handleClarifyAnswer = async (answer) => {
    ...
    if (currentQuestionIndex + 1 < clarifyingQuestions.length) {
      ...
      addBotMessage("Thank you! Compiling the final Evaluation Baseline...", "loading");
  ...
  const startEvaluations = async (allCreatives) => {
    setStep("EVALUATING");
    addBotMessage("All creatives received! The LLM Council is deliberating in parallel. This will take 1-2 minutes...", "loading");
    setIsLoading(true);
    ...
    try {
      ...
      setStep("POST_EVAL");
      setTimeout(() => addBotMessage("Would you like me to generate a head-to-head comparison table across all criteria? (Yes/No)"), 1000);
  ...
  const handlePostEvalResponse = async (text) => {
    addUserMessage(text);
    const isYes = text.toLowerCase().includes("yes") || text.toLowerCase().includes("yep");
    ...
    if (step === "POST_EVAL") {
      if (isYes) {
        ...
        try {
          ...
          const cleanEvals = evaluations.map(e => ({ platform: e.platform, final_effectiveness_index: e.result.final_effectiveness_index, final_report: e.result.final_report, role_evaluations: e.result.role_evaluations }));
          const res = await api.compareCreatives(cleanEvals);
          setMessages(prev => prev.filter(m => m.type !== "loading"));
          addBotMessage(res.comparison_markdown, "markdown");
          
          setStep("ASK_RECOMMENDATIONS");
          setTimeout(() => addBotMessage("Would you like actionable recommendations to make these creatives stronger before launch?"), 1000);
        } catch (err) {
          setMessages(prev => prev.filter(m => m.type !== "loading"));
         ...
         setStep("ASK_RECOMMENDATIONS");
         setTimeout(() => addBotMessage("Alright! Would you like actionable recommendations to make these creatives stronger before launch?"), 1000);
      }
      ...
      if (isYes) {
        ...
        try {
          const cleanEvals = evaluations.map(e => ({ platform: e.platform, final_effectiveness_index: e.result.final_effectiveness_index, role_evaluations: e.result.role_evaluations }));
          const res = await api.getRecommendations(cleanEvals);
          setMessages(prev => prev.filter(m => m.type !== "loading"));
          addBotMessage(res.recommendations_markdown, "markdown");
          
          setStep("DONE");
          setTimeout(() => addBotMessage("All done! If you have further assets or campaigns to evaluate, let me know or start a new session."), 1000);
        } catch (err) {
          setMessages(prev => prev.filter(m => m.type !== "loading"));
          addBotMessage(`Error: ${err.message}`, "error");
        } finally {
  ...
  const handleInputSubmit = (e) => {
    ...
    if (step === "CLARIFY_GAPS") {
      ...
      addUserMessage(text);
      setTimeout(() => addBotMessage("The evaluation workflow is complete. Please start a new session for another campaign."), 500);
    }
  };
    ...
    <div className="wizard-chat-container">
        ...
        {step === "INITIAL_UPLOAD" ? (
          ...
          >
            <div className="hero-content">
              ...
              <p>Drag and drop your creative ad assets (video/image) or a brief anywhere here, or use the button below.</p>
              
              <label className="hero-upload-button">
                <input type="file" onChange={handleInitialUpload} multiple accept="video/*,image/*,.pdf,.doc,.docx" disabled={isLoading} />
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`message-bubble ${msg.role}`}>
                <div className="message-content">
                  ...
                  {msg.type === "upload_initial" && null /* Handled by hero now */}
                  
                  {msg.type === "upload_platform" && (
                    <div className="upload-prompt">
                      <p>{msg.content}</p>
                      <label className="upload-button blue-btn">
                        <input type="file" onChange={(e) => handlePlatformUpload(e, msg.extra.platform)} multiple accept="video/*,image/*" disabled={isLoading} />
      ...
      <form className="chat-input-area" onSubmit={handleInputSubmit}>
        <textarea
          ...
          disabled={isLoading || step === "INITIAL_UPLOAD" || step.includes("UPLOAD_CREATIVES")}
          rows={2}
        />
